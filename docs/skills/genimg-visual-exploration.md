...
3. **Generate.** One `-n` run per provider with your directions as `--deltas`. Each image keeps
...
```bash
genimg "a SINGLE poster about <claim>, NOT a grid" -m gdm:nb2 -n 5 --deltas @directions.txt -o explore/gemini.png
genimg grid explore/*.png -o explore/directions.html --open
```
