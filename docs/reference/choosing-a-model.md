...
- Best for the price: `oai:gi1-mini`, `gdm:nb2-lite`, `gdm:nb2` and `oai:gi2.5`.
- genimg's default `--quality medium` costs $0.013 on `oai:gi2.5`, 16 times less than the scored
  `max`.
...
`--quality auto` lets OpenAI choose; genimg prices it as `medium`. Every price:
[Models](models.md#openai).

## Quality against speed

- Fastest for the quality: `gdm:nb2-lite`, `gdm:nb2`, `oai:gi2.5-flare` and `oai:gi2.5`.
