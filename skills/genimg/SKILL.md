...
- **YOU are the tailored-diversity engine.** genimg deliberately contains no text LLM (design invariant — image models only). For spread beyond the curated `-d` deltas, compose your own n variant prompts (you know the subject and context better than any fixed list) and run them as separate genimg calls, then compare with `genimg grid`. Use `-d` when a quick generic spread is enough.
