...
- **The two combos to reach for**: (1) **`--deltas` (or bare `-d`) in parallel mode — works on ALL providers** and gives engineered spread; (2) **`-d --mode batch` on Gemini** — the model sees all n takes in one request and curates a deliberately different *set* (verified: distinct palettes AND techniques, e.g. line-art / block-print / brush-stroke / tangram from one call). Slower per batch (~30s vs ~8s parallel) but the most coherent collection.
