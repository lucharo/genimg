...
3. If a provider is not authenticated, `genimg auth --modes` names the env vars each auth mode reads. The user puts the key in the environment; never take it in chat or pass it as an argument. Then run `genimg setup` yourself: without a terminal it never prompts, saves each provider whose key validates, and exits 1 if none does. `genimg auth --check` runs a tiny live generation on the Google and OpenAI providers' fixed canary models (Codex: login only), which proves the provider, not the model you pick; adding `--json` skips that probe.
...
| `--aspect-ratio` | `-a` | `1:1 4:3 3:4 16:9 9:16 3:2 2:3 4:5 5:4 21:9`; `gdm:nb2`/`nb2-lite` add `1:4 1:8 4:1 8:1` | OpenAI: `16:9`/`9:16` need `-r 2K` or `4K`; `4:3`/`3:4` need `1K` or `2K`; `gi1`/`gi1-mini`/`gi1.5`: `1:1` only |
| `--resolution` | `-r` | `512 1K 2K 4K` | `gdm:nb2`: all four; `gdm:nbp`: `1K 2K 4K`; `gdm:nb2-lite`: `1K`; OpenAI: `1K 2K 4K` per the aspect rule (`gi1`/`gi1-mini`/`gi1.5`: `1K` only); `codex:image`: none |
| `--quality` | `-q` | `low medium high auto`; `oai:gi2.5`/`gi2.5-flare` add `xhigh max` | OpenAI only; default `medium`; `high` is 30–90 s/image |
| `--thinking` | | `minimal` \| `high` | `gdm:nb2` and `gdm:nb2-lite` only |
