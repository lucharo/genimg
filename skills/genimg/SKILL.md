...
- **Model choice:** default `oai:gi2` (gpt-image-2) — best for legible in-image **text, glyphs, logos, UI**. `-m gdm:nbp` (Gemini 3 Pro) — best for **photographic / painterly quality**, and (with `gdm:nb2`, Gemini 3.1 Flash) **far better than gpt-image-2 at structured diagrams** — layout, arrows, spatial instructions (see *Diagrams & infographics* below). `genimg models` lists all.
