...
- **Codex subscription:** `genimg "PROMPT" -m codex:image -o out.png` uses the user's local Codex CLI and ChatGPT login (`codex login`). No API key. Generation, `-i`, references and `-n`/`-d` work; Codex selects the image model and size. `-a` is a prompt request, not a guaranteed dimension. Omit `-q`, `-r`, `--thinking`, `--auth`, `--region`, `--project` and `--mode batch`. Billing is subscription usage, with no dollar estimate; never label this backend as GPT Image 2.5. It starts an ephemeral Codex agent run, so use the host's native image tool directly when one is already available.
